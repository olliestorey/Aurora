...
const playerModel = ref<PlayerModel>({
  name: "",
  email: "",
  roomCode: "",
});
  ...
  if (playerModel.value.name === "" || playerModel.value.roomCode === "") {
    ...
    const response = await fetch(
      `${runtimeConfig.public.apiBase}/api/room/join`,
      {
        ...
        body: JSON.stringify({
          roomCode: playerModel.value.roomCode,
          playerName: playerModel.value.name,
          playerEmail: playerModel.value.email,
        }),
      }
    );
