<template>
  <div class="leaderboard">
    ...
    <div class="leaderboard-global__results">
      <div
        v-for="(entry, index) in gameLobby.players"
        :key="entry.id"
        class="leaderboard-global__entry cb-black"
      >
        <div class="leaderboard-global__details">
          <span class="leaderboard-global__entry-position cb-yellow--bg">
            {{ index + 1 }}
          </span>
        ...
        <div class="leaderboard-game__score">
          <div
            class="leaderboard-global__entry-progress-bar cb-yellow--bg"
            :style="{
              width:
                (entry.wordsSubmited.length / gameLobby.totalWords) * 100 + '%',
            }"
          >
            {{
              entry.wordsSubmited.length / gameLobby.totalWords !== 0
                ? Math.floor(
                    (entry.wordsSubmited.length / gameLobby.totalWords) * 100
                  ) + "%"
                : ""
      ...
      <NuxtLink to="/" class="leaderboard-global__back-link cb-yellow--bg">
        Back to Main Page
      </NuxtLink>
    </div>
  </div>
